...
use crate::auto::{log_yaml, DynamicUpdate, NamedAsset, StaticUpdate, Update};
use colored::Colorize;

pub fn log_update(update: &Update) -> Result<(), Box<dyn Error>> {
    match update {
        Update::DynamicAgent(update) => match update {
        ...
        Update::StaticAgent(update) => match update {
            StaticUpdate::Plan(plan) => {
                println!(
                    "{} | {}",
                    "Static Agent".yellow().bold(),
                    "Created Plan".white()
                );
                println!();
                log_yaml(&plan)?;
