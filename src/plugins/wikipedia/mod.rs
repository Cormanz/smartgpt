...
use crate::{
    apply_chunks, invoke, BrowseRequest, Command, CommandArgument, CommandContext, CommandImpl,
    CommandNoArgError, EmptyCycle, Plugin, ScriptValue,
};
...
impl Display for WikipediaNoPageError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{}",
            "one of the 'wikipedia' commands did not find a page."
        )
    }
}
...
pub async fn wikipedia_search(
    ...
    let query: String = args
        .get(0)
        .ok_or(CommandNoArgError("wikipedia-search", "query"))?
        .clone()
        .try_into()?;
...
pub async fn wikipedia_get(
    ctx: &mut CommandContext,
    args: Vec<ScriptValue>,
) -> Result<ScriptValue, Box<dyn Error>> {
    let name: String = args
        .get(0)
        .ok_or(CommandNoArgError("wikipedia-browse", "name"))?
        .clone()
        .try_into()?;
    ...
    Plugin {
        ...
        commands: vec![
            Command {
                ...
                return_type: "{ query: { search: { title: String, extract: String }[] } }"
                    .to_string(),
