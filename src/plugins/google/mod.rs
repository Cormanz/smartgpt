...
use crate::{
    invoke, BrowseRequest, Command, CommandArgument, CommandContext, CommandImpl, Plugin,
    ...
    let json = match json_result {
        ...
        Err(err) => {
            ...
            return Ok(ScriptValue::Dict(HashMap::from_iter([(
                "error".to_string(),
                format!(
                    "Unable to parse your Google request for \"{query}\" Try modifying your query \
                     or waiting a bit."
                )
                .into(),
            )])));
    ...
    Plugin {
        ...
        commands: vec![Command {
            ...
            args: vec![CommandArgument::new(
                "query",
                "The request to search. Create a short, direct query with keywords.",
                "String",
            )],
            return_type: "{ items: { title: String, link: String, snippet: String }[] }"
                .to_string(),
