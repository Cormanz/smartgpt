...
use crate::{
    auto::{
        agents::{
            prompt::{
                CreatePlanInfo, NextStepInfo, NoData, PersonalityInfo, SaveAssetInfo, CREATE_PLAN,
                NEXT_STEP, PERSONALITY, SAVE_ASSET, SUMMARIZE_MEMORIES,
            },
            worker::create_tool_list,
        },
        run::Action,
        try_parse_json, DisallowedAction, NamedAsset, StaticUpdate, Update,
    },
    AgentInfo, CommandContext, Message, Tool, Weights,
};
...
pub fn add_memories(
    ...
    agent
        .llm
        .message_history
        .push(Message::User(SUMMARIZE_MEMORIES.fill(NoData)?));

    let memories = try_parse_json::<Memories>(&agent.llm, 2, Some(700), Some(0.5))?.data;
    listen_to_update(&Update::StaticAgent(StaticUpdate::SavedMemories(
        memories.clone(),
    )))?;
...
pub fn run_method_agent(
    ...
    planner
        .llm
        .prompt
        .push(Message::User(CREATE_PLAN.fill(CreatePlanInfo {
            task: task.to_string(),
            observations,
            tools,
            assets: data,
    ...
    planner
        .llm
        .message_history
        .push(Message::Assistant(plan.raw));
    let plan = plan.data;
    ...
    for step in plan.steps {
        ...
        listen_to_update(&Update::StaticAgent(StaticUpdate::SelectedStep(
            step.clone(),
        )))?;

        agent.llm.message_history.push(Message::User(
            NEXT_STEP.fill(NextStepInfo { step: step_text })?,
        ));
        ...
        listen_to_update(&Update::StaticAgent(StaticUpdate::Thoughts(
            thoughts.clone(),
        )))?;
        ...
        let out = use_tool(
            context,
            &|context| &mut context.agents.fast,
            thoughts.action.clone(),
        );
        ...
        match out {
            Ok(out) => {
                ...
                listen_to_update(&Update::StaticAgent(StaticUpdate::ActionResults(
                    out.clone(),
                )))?;
    ...
    for asset in plan.assets {
        listen_to_update(&Update::StaticAgent(StaticUpdate::SelectedAsset(
            asset.name.clone(),
        )))?;
        ...
        let asset_content =
            agent
                .llm
                .model
                .get_response_sync(&agent.llm.get_messages(), Some(800), Some(0.3))?;
        ...
        *context
        ...
        listen_to_update(&Update::StaticAgent(StaticUpdate::AddedAsset(
            named_asset.clone(),
        )))?;
    }
